        ...
        <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-blue-100">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-gray-600 text-sm">Atualize as informações do cliente {{ $client->name }}</p>
                </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="tel" name="phone" value="{{ old('phone', $client->formatted_phone) }}" placeholder="(11) 99999-9999"
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="cpf_cnpj" value="{{ old('cpf_cnpj', $client->formatted_cpf_cnpj) }}" placeholder="000.000.000-00 ou 00.000.000/0000-00"
                        ...
                        <div>
                            ...
                            <input type="text" name="ie_rg" value="{{ old('ie_rg', $client->ie_rg) }}" placeholder="RG ou Inscrição Estadual"
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input id="zip_code" type="text" name="zip_code" value="{{ old('zip_code', $client->formatted_zip_code) }}" placeholder="00000-000"
                        ...
                        <div>
                            ...
                            <input type="text" name="number" value="{{ old('number', $client->number) }}" placeholder="123"
                        ...
                        <div>
                            ...
                            <input type="text" name="complement" value="{{ old('complement', $client->complement) }}" placeholder="Apto, Sala..."
                        ...
                        <div>
                            ...
                            <input type="text" name="codigo_ibge" value="{{ old('codigo_ibge', $client->codigo_ibge) }}" placeholder="4314902"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input id="address" type="text" name="address" value="{{ old('address', $client->address) }}" placeholder="Rua, Avenida..."
                        ...
                        <div>
                            ...
                            <input id="neighborhood" type="text" name="neighborhood" value="{{ old('neighborhood', $client->neighborhood) }}" placeholder="Centro"
                        ...
                        <div>
                            ...
                            <input id="city" type="text" name="city" value="{{ old('city', $client->city) }}" placeholder="São Paulo"
                        ...
                        <div>
                            ...
                            <select id="state" name="state" class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 @error('state') border-red-500 @enderror">
                                ...
                                <option value="AC" {{ old('state', $client->state) == 'AC' ? 'selected' : '' }}>AC - Acre</option>
                                <option value="AL" {{ old('state', $client->state) == 'AL' ? 'selected' : '' }}>AL - Alagoas</option>
                                <option value="AP" {{ old('state', $client->state) == 'AP' ? 'selected' : '' }}>AP - Amapá</option>
                                <option value="AM" {{ old('state', $client->state) == 'AM' ? 'selected' : '' }}>AM - Amazonas</option>
                                <option value="BA" {{ old('state', $client->state) == 'BA' ? 'selected' : '' }}>BA - Bahia</option>
                                <option value="CE" {{ old('state', $client->state) == 'CE' ? 'selected' : '' }}>CE - Ceará</option>
                                <option value="DF" {{ old('state', $client->state) == 'DF' ? 'selected' : '' }}>DF - Distrito Federal</option>
                                <option value="ES" {{ old('state', $client->state) == 'ES' ? 'selected' : '' }}>ES - Espírito Santo</option>
                                <option value="GO" {{ old('state', $client->state) == 'GO' ? 'selected' : '' }}>GO - Goiás</option>
                                <option value="MA" {{ old('state', $client->state) == 'MA' ? 'selected' : '' }}>MA - Maranhão</option>
                                <option value="MT" {{ old('state', $client->state) == 'MT' ? 'selected' : '' }}>MT - Mato Grosso</option>
                                <option value="MS" {{ old('state', $client->state) == 'MS' ? 'selected' : '' }}>MS - Mato Grosso do Sul</option>
                                <option value="MG" {{ old('state', $client->state) == 'MG' ? 'selected' : '' }}>MG - Minas Gerais</option>
                                <option value="PA" {{ old('state', $client->state) == 'PA' ? 'selected' : '' }}>PA - Pará</option>
                                <option value="PB" {{ old('state', $client->state) == 'PB' ? 'selected' : '' }}>PB - Paraíba</option>
                                <option value="PR" {{ old('state', $client->state) == 'PR' ? 'selected' : '' }}>PR - Paraná</option>
                                <option value="PE" {{ old('state', $client->state) == 'PE' ? 'selected' : '' }}>PE - Pernambuco</option>
                                <option value="PI" {{ old('state', $client->state) == 'PI' ? 'selected' : '' }}>PI - Piauí</option>
                                <option value="RJ" {{ old('state', $client->state) == 'RJ' ? 'selected' : '' }}>RJ - Rio de Janeiro</option>
                                <option value="RN" {{ old('state', $client->state) == 'RN' ? 'selected' : '' }}>RN - Rio Grande do Norte</option>
                                <option value="RS" {{ old('state', $client->state) == 'RS' ? 'selected' : '' }}>RS - Rio Grande do Sul</option>
                                <option value="RO" {{ old('state', $client->state) == 'RO' ? 'selected' : '' }}>RO - Rondônia</option>
                                <option value="RR" {{ old('state', $client->state) == 'RR' ? 'selected' : '' }}>RR - Roraima</option>
                                <option value="SC" {{ old('state', $client->state) == 'SC' ? 'selected' : '' }}>SC - Santa Catarina</option>
                                <option value="SP" {{ old('state', $client->state) == 'SP' ? 'selected' : '' }}>SP - São Paulo</option>
                                <option value="SE" {{ old('state', $client->state) == 'SE' ? 'selected' : '' }}>SE - Sergipe</option>
                                <option value="TO" {{ old('state', $client->state) == 'TO' ? 'selected' : '' }}>TO - Tocantins</option>
                            </select>
                            @error('state')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="consumidor_final" class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 @error('consumidor_final') border-red-500 @enderror" required>
                                <option value="N" {{ old('consumidor_final', $client->consumidor_final ?? 'N') == 'N' ? 'selected' : '' }}>Não</option>
                                <option value="S" {{ old('consumidor_final', $client->consumidor_final) == 'S' ? 'selected' : '' }}>Sim</option>
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="observations" rows="3" placeholder="Observações gerais sobre o cliente..."
                                      class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 @error('observations') border-red-500 @enderror">{{ old('observations', $client->observations) }}</textarea>
                            @error('observations')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
