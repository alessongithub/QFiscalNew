<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Notícia</h2>
    </x-slot>
    <div class="bg-white p-4 rounded shadow max-w-3xl">
        <form method="POST" action="{{ route('admin.news.update', $news) }}" enctype="multipart/form-data" class="space-y-3">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm text-gray-600">Título</label>
                <input name="title" value="{{ old('title', $news->title) }}" class="w-full border rounded px-3 py-2" required />
            </div>
            <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
                ...
                <label class="inline-flex items-center mt-6"><input type="checkbox" name="active" value="1" class="mr-2" {{ $news->active ? 'checked' : '' }} /> Ativa</label>
            </div>
