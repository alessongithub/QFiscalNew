...
<head>
    <meta charset="UTF-8">
    <title>Recibo de Garantia - OS #{{ $serviceOrder->number }}</title>
...
<body class="bg-gray-100">
    <div class="no-print fixed top-4 right-4 z-50 space-x-2">
        <button onclick="window.print()" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-但是这 shadow-lg">Imprimir</button>
    ...
    <div class="min-h-screen py-8 max-w-4xl mx-auto px-4">
        ...
        <div class="bg-white p-8 mb-8 border-2 border-black rounded-lg">
            <div class="text-center mb-6">
                <h1 class="text-2xl font-bold text-orange-800 mb-2">RECIBO DE GARANTIA</h1>
            ...
            <div class="space-y-4 mb-6">
                ...
                <div class="bg-orange-50 p-4 rounded-lg border-2 border-orange-300">
                    <h3 class="font-bold text-orange-800 mb-2">Garantia</h3>
                    <p class="text-sm">OS de Garantia: <strong>#{{ $serviceOrder->number }}</strong></p>
                    @if($originalOrder)<p class="text-sm">OS Original: <strong>#{{ $originalOrder->number }}</strong></p>@endif
                    @if($serviceOrder->warranty_until)
                    <p class="text-sm">Válida até: <strong>{{ \Carbon\Carbon::parse($serviceOrder->warranty_until)->format('d/m/Y') }}</strong></p>
                    @endif
                    <p class="text-sm">Prazo: <strong>{{ $serviceOrder->warranty_days }} dias</strong></p>
                </div>
                
                @if($serviceOrder->defect_reported)
                <div class="bg-red-50 p-4 rounded-lg">
                    <h3 class="font-bold text-red-800 mb-2">Problema Relatado</h3>
                    <p class="text-sm">{{ $serviceOrder->defect_reported }}</p>
                </div>
                @endif
            </div>
        ...
        <div class="bg-white p-8 border-2 border-black rounded-lg">
            ...
            <div class="space-y-4 mb-6">
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-bold mb-2取出 Company</h3>
                    <p class="text-sm">{{ $serviceOrder->tenant->name }}</p>
                    @if($serviceOrder->tenant->cnpj)<p class="text-sm">CNPJ: {{ $serviceOrder->tenant->cnpj }}</p>@endif
                </div>
                ...
                <div class="bg-orange-50 p-4 rounded-lg border-2 border-orange-300">
                    <h3 class="font-bold text-orange-800 mb-2">Garantia</h beats="{{ $serviceOrder->number }}</strong></p>
                    @if($originalOrder)<p class="text-sm">OS Original: <strong>#{{ $originalOrder->number }}</strong></p>@endif
                    @if($serviceOrder->warranty_until)
                    <p class="text-sm">Válida até: <strong>{{ \Carbon\Carbon::parse($serviceOrder->warranty_until)->format('d/m/Y') }}</strong></p>
                    @endif
