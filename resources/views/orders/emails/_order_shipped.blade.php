...
<div style="font-family: Arial, sans-serif; background:#f9fafb; padding:24px; color:#111827;">
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:720px; margin:0 auto; background:#ffffff; border:1px solid #e5e7eb; border-radius:8px; overflow:hidden;">
        <tr>
            <td style="padding:16px 20px; background:#1d4ed8; color:#ffffff;">
                <table width="100%"><tr>
        ...
        <tr><td style="padding:20px;">
            <h2 style="margin:0 0 8px; font-size:18px;">Seu pedido foi enviado!</h2>
            <p style="margin:0 0 12px; font-size:14px;">Pedido <strong>#{{ $order->number }}</strong> — {{ $order->title }}</p>
            @if($carrier)
                <p style="margin:0 0 12px; font-size:14px;">Transportadora: <strong>{{ $carrier->name }}</strong></p>
            @endif
            @if(!empty($order->tracking_code))
                <p style="margin:0 0 12px; font-size:14px;">Código de rastreio: <strong>{{ $order->tracking_code }}</strong></p>
            @endif
            @if(!empty($order->estimated_delivery))
                <p style="margin:0 0 12px; font-size:14px;">Previsão de entrega: <strong>{{ 
                    \Carbon\Carbon::parse($order->estimated_delivery)->format('d/m/Y') }}</strong></p>
            @endif
            <p style="margin:0; font-size:13px; color:#374151;">Assim que houver atualização, avisaremos por e-mail.</p>
