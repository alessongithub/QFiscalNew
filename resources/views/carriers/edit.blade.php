<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Transportadora</h2>
            ...
            <div class="border rounded-lg p-4">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <label class="inline-flex items-center gap-2 text-sm">
                        <input type="checkbox" name="active" value="1" class="h-4 w-4" @checked(old('active', $carrier->active))>
                        <span>Ativo</span>
                    </label>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $carrier->name) }}" class="w-full border rounded px-3 py-2 {{ $errors->has('name') ? 'border-red-500' : '' }}" placeholder="Transportadora XPTO" required>
                        @error('name')<div class="text-xs text-red-600 mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">Nome Fantasia</label>
                        <input type="text" name="trade_name" value="{{ old('trade_name', $carrier->trade_name) }}" class="w-full border rounded px-3 py-2" placeholder="XPTO Logística">
                    </div>
                </div>
            </div>
            ...
            <div class="border rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="cnpj" value="{{ old('cnpj', $carrier->cnpj) }}" class="w-full border rounded px-3 py-2" placeholder="00.000.000/0000-00" inputmode="numeric" pattern="^\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}$" title="Formato: 00.000.000/0000-00">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">Inscrição Estadual</label>
                        <input type="text" name="ie" value="{{ old('ie', $carrier->ie) }}" class="w-full border rounded px-3 py-2" placeholder="ISENTO ou número">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">RNTRC</label>
                        <input type="text" name="rntc" value="{{ old('rntc', $carrier->rntc) }}" class="w-full border rounded px-3 py-2" placeholder="Registro RNTRC">
                    </div>
                </div>
            </div>
            ...
            <div class="border rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    <div class="md:col-span-3">
                        ...
                        <input id="zip_code" type="text" name="zip_code" value="{{ old('zip_code', $carrier->zip_code) }}" class="w-full border rounded px-3 py-2" placeholder="00000-000" inputmode="numeric" pattern="^\d{5}-\d{3}$" title="Formato: 00000-000">
                        <div id="cep_help" class="text-[11px] text-gray-500 mt-1"></div>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-xs text-gray-600 mb-1">UF</label>
                        <input id="state" type="text" name="state" value="{{ old('state', $carrier->state) }}" maxlength="2" class="w-full border rounded px-3 py-2" placeholder="UF" pattern="^[A-Z]{2}$" title="Use UF com 2 letras, ex.: SP">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-xs text-gray-600 mb-1">Número</label>
                        <input type="text" name="number" value="{{ old('number', $carrier->number) }}" class="w-full border rounded px-3 py-2" placeholder="Nº">
                    </div>
                    <div class="md:col-span-5">
                        <label class="block text-xs text-gray-600 mb-1">Complemento</label>
                        <input type="text" name="complement" value="{{ old('complement', $carrier->complement) }}" class="w-full border rounded px-3 py-2" placeholder="Sala, Bloco, ...">
                    </div>
                    <div class="md:col-span-6">
                        <label class="block text-xs text-gray-600 mb-1">Logradouro</label>
                        <input id="street" type="text" name="street" value="{{ old('street', $carrier->street) }}" class="w-full border rounded px-3 py-2" placeholder="Rua, Avenida...">
                    </div>
                    <div class="md:col-span-3">
                        <label class="block text-xs text-gray-600 mb-1">Bairro</label>
                        <input id="district" type="text" name="district" value="{{ old('district', $carrier->district) }}" class="w-full border rounded px-3 py-2">
                    </div>
                    <div class="md:col-span-3">
                        <label class="block text-xs text-gray-600 mb-1">Cidade</label>
                        <input id="city" type="text" name="city" value="{{ old('city', $carrier->city) }}" class="w-full border rounded px-3 py-2">
                    </div>
                </div>
            </div>
                ...
                <div class="border rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $carrier->phone) }}" class="w-full border rounded px-3 py-2" placeholder="(00) 00000-0000" inputmode="numeric" pattern="^\(\d{2}\)\s?\d{4,5}-\d{4}$" title="Formato: (00) 00000-0000 ou (00) 0000-0000">
                        </div>
                        <div>
                            <label class="block text-xs text-gray-600 mb-1">Email</label>
                            <input type="email" name="email" value="{{ old('email', $carrier->email) }}" class="w-full border rounded px-3 py-2" placeholder="contato@empresa.com">
                        </div>
                    </div>
                </div>
                <div class="border rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="vehicle_plate" value="{{ old('vehicle_plate', $carrier->vehicle_plate) }}" class="w-full border rounded px-3 py-2" placeholder="AAA0A00" pattern="^([A-Z]{3}-?\d{4}|[A-Z]{3}\d[A-Z]\d{2})$" title="Placa antiga (AAA-0000) ou Mercosul (AAA0A00)">
                        </div>
                        <div>
                            <label class="block text-xs text-gray-600 mb-1">UF Placa</label>
                            <input type="text" name="vehicle_state" value="{{ old('vehicle_state', $carrier->vehicle_state) }}" maxlength="2" class="w-full border rounded px-3 py-2" placeholder="UF" pattern="^[A-Z]{2}$" title="Use UF com 2 letras, ex.: SP">
                        </div>
                    </div>
                </div>
            </div>
